...
def movielookup(mykey, searchstring, typestring = ""):
    """Interactions with OMDB API
       mykey = omdb api key
       searchstring = string to search for"""
    try:
        # begin constructing API
        api = f"{OMDBURL}apikey={mykey}&s={searchstring}&t={typestring}"
    ...
    while True:
        ...
        if answer in ["1", "2"]:
            ...
            if answer == "1":
                ...
                typestring = input("Search all movies in the OMDB. Enter TYPE string: ")
                resp = movielookup(mykey, searchstring, typestring)
